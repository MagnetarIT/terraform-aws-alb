...
module "alb" {
  source                                  = "git::https://github.com/MagnetarIT/terraform-aws-alb.git?ref=tags/0.5.0"
  namespace                               = "mag"
  environment                             = "test"
  name                                    = "app"
  vpc_id                                  = module.vpc.vpc_id
...
| health\_check\_protocol | The protocol to use to connect with the target. Defaults to alb target protocol (target\_group\_protocol) | `string` | `""` | no |
